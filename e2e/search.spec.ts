...
test.describe('Search', () => {
  test('should open search modal when clicking search button', async ({ page }) => {
    ...
    await expect(page.getByTestId('ai-search-modal')).toBeVisible({ timeout: 10000 })
  })

  test('should open search modal with Ctrl+K', async ({ page }) => {
    await page.goto('/', { waitUntil: 'domcontentloaded' })
    // Wait for search button to be visible (ensures JS event listeners are bound)
    await expect(page.getByRole('button', { name: '搜索' })).toBeVisible({ timeout: 10000 })
  ...
  test('should close search modal with ESC', async ({ page }) => {
    ...
    await expect(page.getByTestId('ai-search-modal')).not.toBeVisible({ timeout: 5000 })
  })
