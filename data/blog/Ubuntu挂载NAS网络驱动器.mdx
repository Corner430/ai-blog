...
summary: '文章指出校外向校内服务器传数据不便，NAS可完美解决并实现储算分离。文中介绍通过Samba在Ubuntu挂载NAS及实验室服务器自动挂载、分配限额的方法，外网同步用Cloud Sync不限速不限额，还能搭建Git server。'
---

校外向校内服务器传输数据极为不便，唯有NAS可完美解决问题，并实现**储算分离**
