...
summary: '本文介绍禁用frp的sftp传输且不影响其他方式的方法。方法一分离ssh与sftp端口，使frp无法转发sftp；方法二通过sshd_config的ip规则限制frp连入地址，但存在缺陷（如影响vscode连接），故推荐方法一。'
---

[如何禁用通过frp进行sftp传输文件？](https://github.com/fatedier/frp/issues/3736)
