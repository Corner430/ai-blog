...
summary: '从固件选择到网络配置的 OpenWrt 全流程指南，涵盖刷机、旁路由、IPv6、DDNS、单线多拨和网络唤醒。'
---

![题图](/static/images/blog/OpenWrt完全配置指南/hero.png)

本文是一份系统性的 OpenWrt 配置指南，涵盖固件选择、刷机方法、网络配置、IPv6、DDNS、单线多拨、网络唤醒等内容，帮助从零开始搭建并配置 OpenWrt 软路由。

## 1 固件选择与下载

### 1.1 x86 设备推荐

3865U 软路由价格偏高，预算有限可以考虑：

- **斐讯 N1**：单臂旁路由首选，闲鱼不到一百块
- **N270 小主机**：双千兆网口，闲鱼几十块
...
- [supes.top — 在线定制固件](https://supes.top/?target=x86/64&id=generic)（需自行安装软件包）
...
> **注意**：ImmortalWrt 原版功能较少，推荐使用第三方编译的固件。
...
> 如果有硬盘转 USB 的转接设备，可以直接用同样方式刷入硬盘，之后从该硬盘启动。
...
如果设备已运行 OpenWrt，可以直接通过系统升级或命令行刷入新固件。
...
单臂旁路由只需一个网口，通过 LAN 口与主路由连接，适合斐讯 N1 等单网口设备。

**配置步骤：**

1. 设备开机后回车进入命令行界面
2. 修改网络配置，为旁路由分配静态 IP：
...
4. 设置 LAN 口：
   ...
   - **关闭 DHCP 服务器**
5. 配置 PassWall 等代理工具
6. 将需要代理的设备的**网关和 DNS 服务器**改为旁路由 IP，或在主路由中统一设置
...
1. 修改 LAN 口地址，取消 LAN 口的桥接设置，开启 DHCP
...
1. **WAN 口设置**：在高级设置中启用 IPv6，勾选「获取 IPv6 地址」为「自动」
2. **LAN 口设置**：IPv6 分配长度设为 `64`，在高级设置中配置 IPv6 相关参数
3. **LAN 口 IPv6 设置**：配置路由通告服务（RA）、DHCPv6 服务等
4. **DNS 设置**：取消「禁止解析 IPv6 DNS 记录」的勾选

### 4.2 校园网无 PD 的 IPv6 配置

> 校园网不发 PD，但 WAN 口可以拿到 `2408` 开头的 IPv6 公网地址，且是 `/64` 后缀，可通过中继模式让 LAN 口获取 IPv6 地址。
...
10. 前往 [test-ipv6.com](https://test-ipv6.com/) 验证 IPv6 是否正常
...
1. 在 Cloudflare 中为域名添加一条 **A 记录**，指向公网 IP
2. 获取 Cloudflare API Key：前往 [API Tokens](https://dash.cloudflare.com/profile/api-tokens) 页面，复制 Global API Key
...
> **注意**：如果遇到问题，把子域名改成 `www`，同时修改 Cloudflare 中 A 记录的名称。

### 5.3 URL 转发（免端口访问）

利用 Cloudflare 的页面规则避免「域名:端口号」的访问方式：

1. 在 Cloudflare 中设置**页面规则**（Page Rules），配置 URL 转发
2. 添加一条与页面规则匹配的 DNS 记录

参考视频：[如何随时随地访问家中的软路由和其他设备？](https://youtu.be/Y0HGp94eHGU?t=889)
...
单线多拨是在同一条物理线路上建立多个 PPPoE 连接，尝试实现带宽叠加的技术。

### 6.2 配置方法

1. 在 OpenWrt 中手动创建多个虚拟 WAN 口
2. 每个虚拟 WAN 口使用**不同的 PPPoE 账号**拨号（同一账号无法多拨）
3. 配置负载均衡策略

> **重要**：虚拟 WAN 口的防火墙设置和防火墙区域设置必须正确配置，否则无法正常上网。

### 6.3 实测结论

如果拨不通，可以按照网络接口列表顺序，自上而下重新连接各接口。

经实际测试：由于 ISP 对接口进行了限速，**并没有实现带宽叠加效果**，反而导致网络更不稳定。
...
1. **OpenWrt 端口转发**：在防火墙中添加端口转发规则，开启外网访问
2. **Windows 网络适配器配置**：
   - 打开网络适配器 → 属性 → 配置
   - 在「高级」选项卡中启用 Wake on Magic Packet 等 WOL 选项
   - 在「电源管理」选项卡中勾选「允许此设备唤醒计算机」
3. **BIOS 设置**：进入主板 BIOS，启用 WOL 功能
4. **OpenWrt 唤醒命令**：

```shell
# 将 <mac_addr> 替换为目标电脑网卡的 MAC 地址
etherwake -i eth1 <mac_addr>
```

也可以在 OpenWrt 的 LuCI 界面中使用网络唤醒功能（服务 → 网络唤醒）。
...
**现象**：修改配置后点击「应用」，界面卡在「正在应用更改」，具体卡在 `/etc/config/firewall` 步骤。

**解决方案**：关闭**广告屏蔽大师**插件即可恢复。该问题与广告屏蔽插件干扰防火墙规则加载有关。
