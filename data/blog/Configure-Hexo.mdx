...
sticky: 1
summary: '文章介绍了Hexo博客优化方法，包括添加百度统计、通过sitemap提交提升搜索引擎收录、使用hexo-simple-mindmap插件绘制思维导图，以及域名解析与绑定步骤，助力博客功能完善与访问体验提升。'
---
...
- 配置sitemap路径
  打开本目录下的\_config.yml文件，将`url`修改为博客的首页地址，这是为了保证sitemap.xml文件路径的正确性。
...
- 提交文件
  ...
  进入[百度提交网址](https://ziyuan.baidu.com/?castk=LTE=)，点击用户中心->添加网站，选择HTML标签验证。将出现的那行meta代码复制下来
  ![HTML标签验证](/static/images/blog/html-tag-verification.png)
...
- 绑定到hexo
  1.  找到当前项目，点击Settings->Pages
      ...
      ![绑定到page](/static/images/blog/bindto-page.png)
  2.  在cloudflare->DNS中添加CNAME记录（不要勾选小云朵）
      ![添加CNAME](/static/images/blog/add-cname.png)
...
- 配置文件
  打开根目录下的\_config.yml文件，在末尾加上下述字段。
...
- 效果
  ![效果1](/static/images/blog/effect-screenshot-1.png)

点开后。

![效果2](/static/images/blog/effect-screenshot-2.png)
...
- 主动推送
  ...
  - 安装插件
    ...
    修改站点根目录下的配置文件\_config.yml，添加以下内容：
  ...
  - 准入密钥获取
    ![准入密钥](/static/images/blog/access-key-acquisition.png)

    > 记得检查根目录\_config.yml文件中url的值，必须包含是百度站长平台注册的域名。

  - 配置
    配置根目录下的\_config.yml，加入新的deployer，**这里的”-“,必不可少！ 否则报错。**
