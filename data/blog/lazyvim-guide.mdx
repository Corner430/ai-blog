...
```lua
-- SSH 环境不使用系统剪贴板 provider，由 nvim-osc52 插件处理
vim.opt.clipboard = ""
```

> **注意：** 不要设为 `"unnamedplus"`。SSH 环境下没有系统剪贴板 provider，`unnamedplus` 会导致 Neovim 尝试调用不存在的 provider 而非 OSC52。
...
| kitty              | ✅                                  |
| macOS Terminal.app | ❌                                  |
| tmux               | ⚠️ 需配置 `set -g set-clipboard on` |
...
| tmux 中不工作      | `~/.tmux.conf` 添加 `set -g set-clipboard on` 并 `tmux source-file ~/.tmux.conf` |
| 插件未加载         | `:Lazy` 中搜索 osc52 确认已安装                                                  |
