...
summary: '介绍C语言中清空输入缓冲区的方法，分析fflush(stdin)和rewind(stdin)的局限性（非标准行为），并提供基于逐字符读取的跨平台替代方案。'
---

在C语言中，`fflush(stdin)` 和 `rewind(stdin)` 主要用于清空输入缓冲区。然而，它们并不是标准C的一部分，因此在不同的编译器和平台上可能表现不同。
