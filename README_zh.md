...
<p align="center">
  <img src="public/static/images/readme/hero-banner.png" alt="Corner430 AI 博客" width="720" />
</p>

# Corner430 AI 博客

一个基于 [Next.js](https://nextjs.org/) 和 [Tailwind CSS](https://tailwindcss.com/) 构建的现代博客，集成了[腾讯混元大模型](https://cloud.tencent.com/document/product/1729) AI 能力。基于 [tailwind-nextjs-starter-blog](https://github.com/timlrx/tailwind-nextjs-starter-blog) v2 开发，包含 48 篇博文和一套完整的 AI 工具。

## AI 功能

<p align="center">
  <img src="public/static/images/readme/ai-features.png" alt="AI 功能" width="640" />
</p>

- **AI 摘要** — 自动生成文章摘要，流式输出，结果缓存在 localStorage 中。
- **AI 问答** — 文章页面浮动聊天面板，读者可就当前文章提问。
- **语义搜索** — 基于 embedding 余弦相似度搜索，替代默认 KBar。
- **自动标签** — AI 推荐标签，复选框选择后直接回写到 frontmatter。
- **封面图生成** — 异步任务模型（提交任务 → 轮询状态 → 预览/下载）。
- **写作助手** — AI 驱动的润色和续写，支持流式输出。

所有 AI 功能在未配置 `HUNYUAN_API_KEY` 时均可优雅降级。所有 AI API 端点受**速率限制**保护（每 IP 每分钟 20 次）。

## UI 与阅读体验

<p align="center">
  <img src="public/static/images/readme/ui-features.png" alt="UI 功能" width="640" />
</p>

- **阅读进度条** — 文章页面顶部固定显示滚动进度。
- **浮动目录** — 侧边栏目录，基于 IntersectionObserver 高亮当前章节。
- **阅读时间与字数** — 在文章列表和文章页面显示（如 "1234 字 · 5 分钟"）。
- **文章置顶** — `sticky` frontmatter 字段，支持将重要文章固定在列表顶部。
- **版权声明** — 每篇文章底部 CC BY-NC-SA 4.0 许可声明（中英双语）。
- **页面浏览计数** — 显示来自 Umami 分析 API 的单页浏览量。
- **Live2D 看板娘** — 可切换的动漫角色小部件，状态持久化到 localStorage。
- **点击动画** — 页面任意位置点击时，彩色爱心向上飘浮。
- **图片灯箱** — 基于 medium-zoom 的文章图片点击放大全屏查看。
- **中文本地化** — 所有 UI 文本已翻译为中文（zh-CN）。
- **资源页面** — 按分类整理的精选学习资源（`/resources`）。
- **项目展示** — 项目卡片包含封面图、描述和 GitHub 链接（`/projects`）。

## 管理后台

<p align="center">
  <img src="public/static/images/readme/admin-dashboard.png" alt="管理后台" width="640" />
</p>

通过直接访问 `/admin` 进入（不在公开导航栏中显示）。

- **封面图生成** (`/admin/cover`) — 提交生成任务，轮询进度，预览和下载。
- **自动标签** (`/admin/tags`) — 浏览文章列表，生成 AI 标签建议，写入 frontmatter。
- **写作助手** (`/admin/writing`) — 润色或续写，流式输出结果。

## 架构

<p align="center">
  <img src="public/static/images/readme/architecture.png" alt="架构" width="640" />
</p>
